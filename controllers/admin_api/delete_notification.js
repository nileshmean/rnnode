...
deletenotification = (req, res) => {
   ...
   console.log(data);
    sql = "UPDATE `notifications` SET `deleted_at`=:deleted_at WHERE `id`=:id";
    dbConnection.query(sql, { replacements:data,type: dbConnection.QueryTypes.UPDATE }).then(function (user) {
        console.log(user);
        if (user.length > 0) {
            var response = {
