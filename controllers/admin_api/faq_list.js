...
FaqList = (req, res) => {
    ...
    var sql = "SELECT * FROM `FAQ` WHERE deleted_at IS NULL"

    var countSQL = "SELECT * FROM `FAQ` WHERE deleted_at IS NULL"
    ...
    if (search !== "") {
        sql = sql + "&& question LIKE '%" + search + "%' "
        countSQL = countSQL + "&& question LIKE '%" + search + "%'"
    }
