...
TeamLeaderUserList = (req, res) => {
  ...
  let select_query = "SELECT users.id,users.user_name,users.image as image,users.password,users.status,users.price_per_hour,GROUP_CONCAT(users_meta.meta_value SEPARATOR ' ') AS name FROM `users` JOIN users_meta ON users.id = users_meta.user_id WHERE type='leader' ";
  let count_query = "SELECT COUNT(users.id) as total_count,users.id,users.user_name,users.password,users.status,users.price_per_hour,GROUP_CONCAT(users_meta.meta_value) AS name FROM `users` JOIN users_meta ON users.id = users_meta.user_id WHERE type='leader'";
  if (search != '') {
...
module.exports = TeamLeaderUserList;
