...
AddProvider = (req, res) => {
	...
	var data = { public_id: public_id, first_name: firstName, last_name: lastName, email: email, image: prifileImage, country_code: country_code, phone: phone, password: password, user_type: 'provider', address: address, location_lat: latitude, location_lng: longitude, city: city, state: state, hydra_facial_machine_id: hydrafacialMachineId, zip_code: zip_code, spa_location: spa_location, spa_location_lat: spa_location_lat, spa_location_lng: spa_location_lng, spa_name: spa_name };

	var query = "INSERT INTO users(public_id, first_name, last_name, email, password, phone, country_code, address, location_lat, location_lng, city, state, zip_code, spa_location, spa_location_lat, spa_location_lng, spa_name, type, hydra_facial_machine_id, image) VALUES (:public_id, :first_name, :last_name, :email, :password, :phone, :country_code, :address, :location_lat, :location_lng, :city , :state, :zip_code, :spa_location, :spa_location_lat, :spa_location_lng, :spa_name, :user_type, :hydra_facial_machine_id, :image )";
	...
	.then(result => {
		if (result.length > 0) {
			...
			.then(result => {
				...
				response.message = constants.PROVIDER_REGISTER_SUCCESS;
				res.send(response);
			})
			.catch(err => {
				console.error("Error into add_provider.js"+ err);
