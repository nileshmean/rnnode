...
EditAesthetician = (req, res) => {
	...
	if (oldPrifileImage != "") {
        var image_path = constants.APP_PATH+'public/uploads/aesthetician/' + oldPrifileImage;
        fs.access(image_path, error => {
            if (!error) {
                ...
                // The check failed.
                console.error("Error into edit_aesthetician.js: ", error);
	...
	.then(result => {
		...
		response.message = constants.AESTHETICIAN_EDIT_SUCCESS;
		res.send(response);
	})
	.catch(err => {
		console.error("Error into edit_aesthetician.js"+ err);
		res.statusCode = constants.ErrorStatusCode;
		response.message = constants.SomethingWentWrong;
		res.send(response);
	});
}
module.exports = EditAesthetician;