...
ProviderList = (req, res) => {
	...
	var query = "SELECT id, first_name, last_name, CONCAT(first_name, ' ', last_name) AS fullname, email, image, status FROM admin WHERE user_type = 'sub_admin' AND deleted_at IS NULL";

	var que_cnt = "SELECT id, first_name, last_name, CONCAT(first_name, ' ', last_name) AS fullname, email, image, status FROM admin WHERE user_type = 'sub_admin' AND deleted_at IS NULL";
	...
	if (search_value != "") {
		query += " HAVING (fullname LIKE '%"+search_value+"%' OR email LIKE '%"+search_value+"%')";
		que_cnt += " HAVING (fullname LIKE '%"+search_value+"%' OR email LIKE '%"+search_value+"%')";
	}
	...
	.then(resul => {
		if (resul.length > 0) {
			...
			.then(result_cnt => {
				...
				response.message = "Sub Admins List.";
	...
	.catch(err => {
		console.error("Error in sub_admin_list.js "+ err);
