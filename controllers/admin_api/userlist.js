...
Userlist = (req, res) => {
  ...
  var type= req.body.type ?req.body.type:"customer";
  ...
  let select_query = "SELECT users.id,users.user_name,users.image as image,users.status,GROUP_CONCAT(users_meta.meta_value SEPARATOR ' ') AS name FROM `users` JOIN users_meta ON users.id = users_meta.user_id WHERE type='"+type+"' ";
  
  let count_query = "SELECT COUNT(users.id) as total_count,users.id,users.user_name,users.status,GROUP_CONCAT(users_meta.meta_value) AS name FROM `users` JOIN users_meta ON users.id = users_meta.user_id WHERE type='"+type+"'";
  if (search != '') {
